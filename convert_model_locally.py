...
model.load_weights("DeepVOG_weights.h5")
#model.save("full.h5")
#model = load_model('full.h5', compile=False)
#tfjs.converters.save_keras_model(model, "./")

##tf.enable_eager_execution()
## Converting a tf.Keras model to a TensorFlow Lite model.
#converter = tf.compat.v1.lite.TFLiteConverter.from_keras_model_file('full.h5')
converter = tf.lite.TFLiteConverter.from_keras_model_file('full.h5')
## REQUIRES TF 1.15 nightly (or newer I guess); 1.15rc1 is too old.

converter.optimizations = [tf.lite.Optimize.DEFAULT]
#converter.target_spec.supported_types = [tf.lite.constants.FLOAT16]
tflite_model = converter.convert()
open("net_fp16.tflite", "wb").write(tflite_model)
